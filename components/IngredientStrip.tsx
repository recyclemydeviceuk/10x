  ...
  return (
    <li className={`group relative aspect-square overflow-hidden border border-paper-200 shadow-card ${className}`}>
      <Image
        ...
        alt={ing.name}
        fill
        sizes="(min-width: 768px) 180px, 48vw"
        className="object-cover transition-transform duration-500 ease-out group-hover:scale-105"
      />
      <div
        className="absolute inset-x-0 bottom-0 flex h-[30%] items-center justify-start border-t border-white/60 px-3 sm:px-4"
        style={{
      ...
      >
        <p className="text-left font-quantico text-[10px] font-bold uppercase leading-tight tracking-tight text-ink sm:text-caption sm:tracking-wide">
          {ing.name}
        </p>
      </div>
    </li>
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-7xl px-5 sm:px-8 md:px-14">
        ...
        {/* Mobile / tablet: swipeable carousel, 2 per view */}
        <ul className="mt-7 flex snap-x snap-mandatory gap-3 overflow-x-auto pb-2 md:hidden [scrollbar-width:none]">
          {INGREDIENT_DETAILS.map((ing) => (
            <Card key={ing.name} ing={ing} className="w-[47%] shrink-0 snap-start" />
          ))}
        </ul>
