  ...
  return (
    ...
    >
      <span className="flex w-full items-center gap-2">
        ...
        >
          {active && <span className="h-1.5 w-1.5 rounded-full bg-ink dark:bg-ink" />}
        </span>
        <span className="font-nebula text-[11px] font-bold uppercase tracking-[0.1em] text-ink dark:text-white md:text-xs">
          {label}
        </span>
      </span>
      <span className="flex flex-wrap items-baseline gap-2 pl-6">
        <span className="font-quantico text-lg font-bold text-ink dark:text-white">{price}</span>
