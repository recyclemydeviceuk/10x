  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        onKeyDown={onKeyDown}
        className={`flex w-full cursor-pointer items-center justify-between gap-3 border bg-white dark:bg-paper px-4 py-3.5 text-left transition-colors ${
          error
            ? 'border-danger'
            : open
              ? 'border-ink'
        ...
        <span className="min-w-0">
          <span
            className={`block truncate font-pt text-[15px] ${
              selected ? 'text-ink dark:text-white' : 'text-fg-subtle'
            }`}
          >
      ...
      {open && (
        <ul
          ...
          tabIndex={-1}
          className="absolute z-30 mt-1 max-h-[19rem] w-full overflow-y-auto border border-ink bg-white dark:bg-paper shadow-elevated"
        >
            ...
            return (
              <li
                ...
                className={`relative cursor-pointer border-b border-paper-100 px-4 py-3 pl-5 last:border-b-0 ${
                  isActive ? 'bg-paper-50 dark:bg-paper-200' : 'bg-white dark:bg-paper'
                }`}
              >
                ...
                <span className="flex items-start justify-between gap-3">
                  <span className="min-w-0">
                    <span className="block font-pt text-[15px] font-bold text-ink dark:text-white">
                      {option.label}
                    </span>
                    {option.hint && (
