...
function Battery({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 48 78"
      className={`text-[#000204] dark:text-white ${className}`}
      role="img"
      aria-label="A battery at full charge"
    >
      {/* terminal */}
      <rect x="17" y="0" width="14" height="6" rx="1.5" fill="currentColor" />
      {/* body */}
      <rect x="0" y="6" width="48" height="72" rx="4" fill="currentColor" />
      {/* charge */}
  ...
  return (
    <div className="flex flex-col items-center gap-6 sm:flex-row sm:items-center sm:gap-5 md:gap-7">
      ...
      <ul className="grid w-full grid-cols-2 gap-x-5 gap-y-7 sm:flex-1">
        {ITEMS.map((item) => (
          <li key={item.unit} className="flex flex-col">
            ...
            <p className={`type-d3 ${item.accent ? 'text-accent-pressed' : 'text-ink dark:text-white'}`}>
              {item.count}
            </p>
            <p className="type-b2 mt-1 font-bold text-ink dark:text-white">{item.unit}</p>
            <p
            ...
            <Image
              ...
              className="mt-3 h-auto w-full dark:brightness-90"
            />
          </li>
        ))}
      </ul>
      ...
      <div aria-hidden className="flex shrink-0 flex-col gap-1.5 sm:gap-2">
        <span className="block h-1.5 w-8 bg-ink dark:bg-white sm:h-2 sm:w-6 md:w-8" />
        <span className="block h-1.5 w-8 bg-ink dark:bg-white sm:h-2 sm:w-6 md:w-8" />
      </div>
      ...
      <div className="flex shrink-0 flex-col items-center gap-3">
        ...
        <Image
          ...
          className="h-auto w-[84px] md:w-[132px] dark:brightness-90"
        />
      </div>
    </div>
  );
