...
const SHORTCUTS = [
  { label: 'Track an order', href: '/account/orders', hint: 'Sign in to see live tracking' },
...
export default async function QueriesPage() {
  ...
  return (
    <main id="main" className="bg-white dark:bg-paper">
      {/* Header band */}
      <section className="border-b border-paper-200 bg-paper-50 dark:bg-paper-100">
        <div className="mx-auto max-w-7xl px-5 pb-12 pt-28 sm:px-8 md:px-14 md:pb-16 md:pt-36">
          <p className="type-k text-ink-800 dark:text-paper-200">Queries</p>
          <h1 className="type-d1 mt-5 max-w-3xl text-ink dark:text-white">
            Ask Us
            <br />
            Anything.
          </h1>
          <p className="type-b1 mt-6 max-w-xl text-ink dark:text-white">
          ...
          <aside className="lg:pt-2">
            ...
            <ul className="mt-5 space-y-6">
              {ASSURANCES.map((item) => (
                <li key={item.title} className="border-l-2 border-accent pl-5">
                  <p className="font-quantico text-[13px] font-bold uppercase tracking-[0.1em] text-ink dark:text-white">
                    {item.title}
                  </p>
            ...
            <ul className="mt-4 space-y-2">
              {SHORTCUTS.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className="group flex items-center justify-between gap-3 border border-paper-200 px-4 py-3.5 transition-colors hover:border-ink dark:hover:border-white"
                  >
                    <span className="min-w-0">
                      <span className="block font-quantico text-[12px] font-bold uppercase tracking-[0.1em] text-ink dark:text-white">
                        {item.label}
                      </span>
            ...
            <p className="type-b2 mt-8 text-fg-subtle">
              ...
              <a
                ...
                className="cursor-pointer text-ink dark:text-white underline decoration-paper-300 underline-offset-2 transition-colors hover:decoration-ink dark:hover:decoration-white"
              >
                {supportEmail}
              </a>
            </p>
